...
function extractFeatures(searchResult) {
    ...
    searchResult.data = searchResult.data == null  ? {} : searchResult.data;
...
function findMemory(str, param) {
    ...
    switch (param) {
        ...
        //Ищем разъем памяти
        case 'port':
            let type = str.match(/(\sDIMM|SO-DIMM)/i)[1];
            let pin = str.match(/(one\s|two\s|three\s|\d\sx\s)(\d+)-pin/i);
            let pinCount  = nums.indexOf(pin[1].toLowerCase()) >= 0 ? (nums.indexOf(pin[1].toLowerCase()) + 1) + ' x ' : pin[1];
            let pinNum = pin[2];
            return type ? `${pinCount}${pin[2]}-конт. ${type}` : '';
            break;
        //Ищем частоту памяти
        case 'freq':
            let pattern = /(\d{3,4}|\d{3,4}\/\d{3,4}|\d{3,4}\/\d{3,4}\/\d{3,4})(\sMHz|MHz|\s)/i;
            let res = str.match(pattern);
        ...
        //Ищем тип памяти
        case 'type':
            let pattern = /(DDR\d|DDR|DDR\dL)/i;
            let res = str.match(pattern);
            if(res !==null) return res[1] ? res[1] : '';
            return '';
            break;
...
function findHumidity(str) {
    let res = srt.match(/(\d{1,2}.+\d{1,2}%)/i);
